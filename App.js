class App {
    ...
    this.$postSection.innerHTML = this.posts.map(post => `
      <div class="post" data-post-id="${post.id}">
        <div class="post-header">
          <div class="post-avatar" style="background-image: url(${post.avatar || 'https://via.placeholder.com/150'})"></div>
          <span class="post-username">${post.username}</span>
          <span class="post-time">${this.formatDate(post.timestamp?.toDate())}</span>
        </div>
        <div class="post-image">
          <img src="${post.image}" alt="Post" onerror="this.src='https://via.placeholder.com/468x585'">
        </div>
  ...
  displayProfilePosts() {
    ...
    this.$profilePosts.innerHTML = userPosts.map(post => `
      <img src="${post.image}" alt="Post" class="profile-post-image" onerror="this.src='https://via.placeholder.com/128'">
    `).join('');
  }
